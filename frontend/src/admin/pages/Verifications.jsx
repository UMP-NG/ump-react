...
const STATUS_STYLE = {
  ...
  conflict: { bg: '#fff7ed', color: '#c2410c', label: 'Conflict' },
};
...
export default function Verifications() {
  ...
  return (
    <div style={{ display: 'flex', gap: 20, height: 'calc(100vh - 64px)', overflow: 'hidden' }}>
      ...
      {/* Left — list */}
      <div style={{ flex: '0 0 380px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '20px 20px 12px', flexShrink: 0 }}>
          ...
          {/* Filter tabs */}
          <div style={{ display: 'flex', gap: 4, background: 'var(--surface)', borderRadius: 'var(--r-pill)', padding: 4 }}>
            {['pending', 'conflict', 'approved', 'rejected', 'all'].map((s) => (
              <button key={s} type="button"
                onClick={() => { setFilter(s); setSelected(null); }}
                style={{ flex: 1, padding: '7px 0', borderRadius: 'var(--r-pill)', fontSize: '1.1rem', fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)', background: filter === s ? 'var(--white)' : 'transparent', color: filter === s ? 'var(--ink-1)' : 'var(--ink-3)', boxShadow: filter === s ? '0 1px 6px rgba(0,0,0,.08)' : 'none', textTransform: 'capitalize', transition: 'all .15s' }}
              >{s}</button>
            ))}
          </div>
        </div>
          ...
          {loading ? (
            ...
            return (
              <button key={r._id} type="button"
                onClick={() => { setSelected(r._id); setNote(''); }}
      ...
      {/* Right — detail */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 20px 40px', borderLeft: '1px solid var(--line)' }}>
        {!sel ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
              ...
              <div>
                <div style={{ fontSize: '1.8rem', fontWeight: 800 }}>{sel.user?.name}</div>
                <div style={{ fontSize: '1.2rem', color: 'var(--ink-3)', marginTop: 2 }}>{sel.user?.email}</div>
              </div>
              <span style={{ marginLeft: 'auto', padding: '4px 12px', borderRadius: 20, fontWeight: 700, fontSize: '1.2rem', background: (STATUS_STYLE[sel.status] || STATUS_STYLE.pending).bg, color: (STATUS_STYLE[sel.status] || STATUS_STYLE.pending).color }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[
                ...
                { label: 'Matric Number', value: sel.matricNumber },
            ...
            {/* Conflict notice */}
            {sel.conflictWith && (
              <div style={{ padding: '12px 14px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 'var(--r-md)', marginBottom: 16, display: 'flex', gap: 10 }}>
                <i className="fas fa-triangle-exclamation" style={{ color: '#f97316', flexShrink: 0, marginTop: 2 }} />
                <div>
                  <div style={{ fontWeight: 700, fontSize: '1.3rem', color: '#9a3412' }}>Conflicts with an existing verified account</div>
                  <div style={{ fontSize: '1.2rem', color: '#c2410c', marginTop: 3 }}>
            ...
            {/* Dispute text */}
            {sel.disputeReason && (
              <div style={{ padding: '12px 14px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 'var(--r-md)', marginBottom: 16 }}>
                <div style={{ fontWeight: 700, fontSize: '1.3rem', color: '#991b1b', marginBottom: 4 }}><i className="fas fa-flag" style={{ marginRight: 6 }} />Dispute statement</div>
            ...
            {sel.documentUrl && (
              <div style={{ marginBottom: 20 }}>
                ...
                <img src={sel.documentUrl} alt="School document" style={{ width: '100%', maxHeight: 420, objectFit: 'contain', borderRadius: 'var(--r-lg)', border: '1px solid var(--line)', background: 'var(--surface)', cursor: 'pointer' }}
                  onClick={() => window.open(sel.documentUrl, '_blank', 'noopener')} />
                <div style={{ fontSize: '1.1rem', color: 'var(--ink-3)', marginTop: 6 }}>Click image to open full size</div>
              </div>
            )}
