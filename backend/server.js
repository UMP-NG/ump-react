...
// ── Cluster mode ──────────────────────────────────────────────────────────────
// Activated by setting CLUSTER=true in production. Requires REDIS_URL so
// Socket.io state is shared across all workers via the Redis adapter.
// Without REDIS_URL, cluster mode is silently disabled to avoid broken sockets.
...
if (ENABLE_CLUSTER && cluster.isPrimary) {
  ...
  // Activated when REDIS_URL is set. Required for cluster mode so every worker
  // can reach sockets on any other worker. Also useful on a single instance to
  // persist online-user state across restarts.
  if (process.env.REDIS_URL) {
    (async () => {
      try {
        ...
        // Parse the URL manually — ioredis can be fussy about non-standard schemes
        // like rediss:// when the URL is passed as a plain string with extra options.
        const u = new URL(process.env.REDIS_URL.trim());
        ...
        const redisOpts = {
          ...
          tls:         (!isLocal || u.protocol === "rediss:") ? {} : undefined,
          lazyConnect: true,
        };
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async (data) => {
      try {
        ...
        // cap text length so the socket can't bypass the HTTP body-size limit
  ...
  mongoose
    ...
    .then(() => {
      ...
      server.listen(PORT, () => {
        ...
        console.log(`✅ Server running on port ${PORT}${workerTag}`);
